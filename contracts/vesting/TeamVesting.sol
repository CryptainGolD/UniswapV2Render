...
contract TeamVesting is Ownable {
    ...
    constructor(address _token, uint256 _totalAmount, address _initialOwner) Ownable(_initialOwner) {
    ...
    function getPendingReward(address _account) public view returns (uint256) {
        ...
        uint256 userReward = users[msg.sender].weight * (totalAmount) * (cycle) / (CYCLE_TIMES) / (totalWeight);
        return userReward - (users[msg.sender].historyAmount);
    }
