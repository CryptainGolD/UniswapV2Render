...
import "../interfaces/IMochaRouter01.sol";
import "./MochaRouter.sol";
...
contract MochaZap is Ownable, IMochaZap, ReentrancyGuard {
    ...
    constructor(address _WNATIVE, address initialOwner) Ownable(initialOwner) {
       WNATIVE = _WNATIVE;
    }
    ...
    function zapInToken(
        ...
        address routerAddr, 
        address _recipient
    ) external override nonReentrant inWhilteList(routerAddr) {
    ...
    function estimateZapInToken(
        ...
        if (_from == IMochaPair(_to).token0() || _from == IMochaPair(_to).token1()) { // check if we already have one of the assets
            // if so, we're going to sell half of _from for the other token we need
            // figure out which token we need, and approve
            address other = _from == IMochaPair(_to).token0() ? IMochaPair(_to).token1() : IMochaPair(_to).token0();
            // calculate amount of _from to sell
            uint sellAmount = _amt / (2);
            // execute swap
            uint otherAmount = _estimateSwap(_from, sellAmount, other, _router);
            if (_from == IMochaPair(_to).token0()) {
            ...
            if (WNATIVE == IMochaPair(_to).token0()) {
                return (nativeAmount / (2), _estimateSwap(WNATIVE, nativeAmount / (2), IMochaPair(_to).token1(), _router ));
            }
            if (WNATIVE == IMochaPair(_to).token1()) {
                return (_estimateSwap(WNATIVE, nativeAmount / (2), IMochaPair(_to).token0(), _router ), nativeAmount / (2));
            }
                return (_estimateSwap(WNATIVE, nativeAmount / (2), IMochaPair(_to).token0(), _router ), _estimateSwap(WNATIVE, nativeAmount / (2), IMochaPair(_to).token1(), _router));
        }
    }
    ...
    function zapIn(
        ...
        address routerAddr, 
        address _recipient
    ) external payable override nonReentrant inWhilteList(routerAddr) {
    ...
    function zapAcross(
        ...
        address _toRouter, 
        address _recipient
    ) external override nonReentrant inWhilteList(_toRouter) {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);

        IMochaPair pair = IMochaPair(_from);
        ...
        IMochaRouter01(_toRouter).addLiquidity(pair.token0(), pair.token1(), amt0, amt1, 0, 0, _recipient, block.timestamp);
    }
    ...
    function zapOut(
        ...
        // get pairs for LP
        address token0 = IMochaPair(_from).token0();
        address token1 = IMochaPair(_from).token1();
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtETH;
            (amtToken, amtETH) = IMochaRouter01(routerAddr).removeLiquidityETH(token, amount, 0, 0, address(this), block.timestamp);
            ...
            uint amt0;
            uint amt1;
            (amt0, amt1) = IMochaRouter01(routerAddr).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
    ...
    function zapOutToken(
        ...
        IERC20(_to).safeTransfer(_recipient, amt0 + (amt1));
    }
    ...
    function _swapTokenToLP(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == IMochaPair(_to).token0() || _from == IMochaPair(_to).token1()) { // check if we already have one of the assets
            ...
            uint liquidity;
            ( , , liquidity) = IMochaRouter01(routerAddr).addLiquidity(_from, other, amount - (sellAmount), otherAmount, 0, 0, recipient, block.timestamp);
            return liquidity;
        } else {
            // go through native token for highest liquidity
    ...
    function _swapNativeToLP(
        ...
        address routerAddress
            ...
            // LP
            IMochaPair pair = IMochaPair(_LP);
    ...
    function _swapHalfNativeAndProvide(
        ...
        address recipient
            ...
            IMochaRouter01 router = IMochaRouter01(routerAddress);
            return router.addLiquidityETH{value : amount - (swapValue)}(token, tokenAmount, 0, 0, recipient, block.timestamp);
    }
    ...
    function _swapNativeToEqualTokensAndProvide(
        ...
        address recipient
            ...
            uint token1Amount = _swapNativeForToken(token1, amount - (swapValue), address(this), routerAddress);
    ...
    function _swapNativeForToken(
        ...
        address[] memory path;
        IMochaRouter01 router = IMochaRouter01(routerAddr);
    ...
    function _swapTokenForNative(
        ...
        address[] memory path;
        IMochaRouter02 router = IMochaRouter02(routerAddr);
    ...
    function withdrawSwapMining(ITradingPool swapIns, uint256 pid) external onlyOwner {
